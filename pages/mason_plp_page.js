...
exports.MasonPLPPage = class MasonPLPPage {
    ...
    async validatePresenceOfFilter() {
        ...
        for (const filterContainer of filterContainers) {
            ...
            if (filterTitle) {
                // Get the text content of the filter title
                const titleText = await filterTitle.textContent();
                const searchBar = await filterContainer.$(filter_search);
                expect(searchBar).toBeTruthy(`Search bar for filter "${titleText.trim()}" is not present.`);
            }
        }
    }
    ...
    async validateFilterExpandClose() {
        ...
        // Iterate through each filter button
        for (const button of filterButtons) {
            // Click the filter button to expand
            //await button.click();

            // Wait for the filters to expand
            await this.page.waitForSelector(`[aria-expanded="true"]`, { timeout: 2000 });

            // Verify if the filters are expanded
            const isExpanded = await button.evaluate(button => button.getAttribute('aria-expanded') === 'true');
            expect(isExpanded).toBeTruthy();
            ...
            // Wait for the filters to collapse
            await this.page.waitForSelector(`[aria-expanded="false"]`, { timeout: 2000 });

            // Verify if the filters are collapsed
            const isCollapsed = await button.evaluate(button => button.getAttribute('aria-expanded') === 'false');
            expect(isCollapsed).toBeTruthy();
        }
    }
